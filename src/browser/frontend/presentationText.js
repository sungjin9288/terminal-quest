import { itemNames } from '../generated/browserData.generated.js';
...
const itemNameCache = new Map(
  Object.entries(itemNames).map(([itemId, name]) => [itemId, stripTrailingVariant(name)])
);

function stripTrailingVariant(text) {
...
export function getPresentationClassLabel(characterClass) {
  return CLASS_LABELS[characterClass] ?? characterClass;
}

export function getPresentationDisplayName(rawName) {
  return stripTrailingVariant(rawName);
}

export function getPresentationLocationDescription(locationId, fallback) {
  return LOCATION_DESCRIPTIONS[locationId] ?? fallback;
}

export function getPresentationShopGreeting(shopId, fallback) {
  return SHOP_GREETING_COPY[shopId] ?? fallback;
}

export function getPresentationSkillCopy(skillId, fallbackName, fallbackDescription) {
...
export function getPresentationItemCopy(input) {
  const itemName = itemNameCache.get(input.itemId) ?? stripTrailingVariant(input.rawName);
  const description =
