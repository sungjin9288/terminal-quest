...
} from '../dist/systems/aiOpsLinearDraft.js';
...
const REPORTS_DIR = path.join(process.cwd(), 'docs', 'ai-linear-drafts');
...
function main() {
  ...
  if (DRY_RUN) {
    console.log('[ai-linear] dry-run mode');
  ...
  console.log('[ai-linear] draft generated');
  console.log(`- telemetry: ${telemetryPath}`);
  console.log(`- notes: ${notesDir}`);
...
try {
  ...
  console.error(`[ai-linear] failed: ${message}`);
  process.exitCode = 1;
}
