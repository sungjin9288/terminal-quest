...
import {
  ...
  deriveAiOpsBacklog
} from '../dist/systems/aiOpsBacklog.js';
...
const REPORTS_DIR = path.join(process.cwd(), 'docs', 'ai-backlog');
...
function main() {
  ...
  if (DRY_RUN) {
    console.log('[ai-backlog] dry-run mode');
  ...
  console.log('[ai-backlog] draft generated');
  console.log(`- telemetry: ${telemetryPath}`);
  console.log(`- notes: ${notesDir}`);
...
try {
  ...
  console.error(`[ai-backlog] failed: ${message}`);
  process.exitCode = 1;
}
